// Project: XnaShooter, File: PostScreenMenu.fx
...
string description = "Post screen shader for the menu in XnaShooter with glow and various screen effects";

// Glow/bloom with menu effects, adjusted for XnaShooter.
...
float HighlightIntensity <
    ...
    float UIMax = 1.0f;
    float UIStep = 0.01f;
> = 0.245f;//0.4f;//0.25f;//1.0f;//0.9f;//0.75f;// 0.5f;
	...
	float2 texCoord : TEXCOORD0)
{
	...
	Out.pos = pos;
	/*test, ok in FXComposer, too small in game!
	float2 texelSize = (BlurWidth /2) / windowSize;

	Out.texCoord[0] = texCoord + texelSize*(float2(1, 1)+direction*(-1.5f));
	Out.texCoord[1] = texCoord + texelSize*(float2(1, 1)+direction*(-0.5f));
	Out.texCoord[2] = texCoord + texelSize*(float2(1, 1)+direction*(+0.5f));
	Out.texCoord[3] = texCoord + texelSize*(float2(1, 1)+direction*(+1.5f));
	*/
...
>
{
	...
	>
	{
		...
		PixelShader  = compile ps_2_0 PS_DownSample20(sceneMapSampler);//radialSceneMapSampler);
	} // pass DownSample
